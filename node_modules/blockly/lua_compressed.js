...
d=b.valueToCode(a,"FROM",Y.NONE)||"0",e=b.valueToCode(a,"TO",Y.NONE)||"0",f=b.valueToCode(a,"BY",Y.NONE)||"1",g=b.statementToCode(a,"DO");g=b.addLoopTrap(g,a);g=is(g,b.INDENT);a="";if($.df(d)&&$.df(e)&&$.df(f))var h=(Number(d)<=Number(e)?"":"-")+Math.abs(Number(f));else a="",h=b.nameDB_.getDistinctName(c+"_inc",$.P.VARIABLE),a+=h+" = ",a=$.df(f)?a+(Math.abs(f)+"\n"):a+("math.abs("+f+")\n"),a=a+("if ("+d+") > ("+e+") then\n")+(b.INDENT+h+" = -"+h+"\n"),a+="end\n";return a+("for "+c+" = "+d+", "+e+
